    ...
    def draw(self, image):
        ...
        def draw_endpoint(x, y, color):
            ...
            image.setPixel(
                ...
                py + 1,
                Color(
                    color.r(), color.g(), color.b(), int(color.a() * fpart(yEnd) * xGap)
                ),
            )
            return px
        ...
        for x in range(xstart, xend):
            ...
            if steep:
                image.setPixel(
                    ...
                    x,
                    Color(
                        color.r(), color.g(), color.b(), int(color.a() * rfpart(yLine))
                    ),
                )
                image.setPixel(
                    y + 1,
                    x,
                    Color(
                        color.r(), color.g(), color.b(), int(color.a() * fpart(yLine))
                    ),
                )
            else:
                image.setPixel(
...
class Triangle(object):
    ...
    def contains_point(self, point):
        ...
        if alpha >= 0 and beta >= 0 and gamma >= 0:
            ...
            red = int(
                alpha * self.p0.color.r()
                + beta * self.p1.color.r()
                + gamma * self.p2.color.r()
            )
            green = int(
                alpha * self.p0.color.g()
                + beta * self.p1.color.g()
                + gamma * self.p2.color.g()
            )
            blue = int(
                alpha * self.p0.color.b()
                + beta * self.p1.color.b()
                + gamma * self.p2.color.b()
            )
            transparency = int(
                alpha * self.p0.color.a()
                + beta * self.p1.color.a()
                + gamma * self.p2.color.a()
            )
    ...
    def draw(self, image, zBuffer):
        ...
        for x in range(xmin, xmax + 1):
            ...
            for y in range(ymin, ymax + 1):
                point_in_triangle, color, zValue = self.contains_point(
    ...
    def draw_faster(self, image, zBuffer):
        ...
        for y in range(ymin, ymax + 1):
            ...
            for edge_start, edge_end in [
                ...
                if (edge_start.y <= y <= edge_end.y) or (
                    edge_end.y <= y <= edge_start.y
                ):
